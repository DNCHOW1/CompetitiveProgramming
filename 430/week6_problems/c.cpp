...
ll greedy(vv64 mat, int parity){
    ll sm = 0;
    int par[2] = {parity, !parity};
    for(auto i = 1; i < mat.size(); i++){
        for(auto j = 1; j < mat[0].size(); j++){
            ...
            if(mat[i][j] == 0){
                ...
                if((mat[i][j]&1) ^ par[i%2]) mat[i][j]++;
            } else{
                // if((mat[i][j]&1) ^ par[i%2] || mat[i][j] <= mat[i-1][j] || mat[i][j] <= mat[i][j-1]) {
...
ll greedy2(vv64 mat, int parity){
    ll sm = 0;
    int par[2] = {parity, !parity};
    for(auto i = 1; i < mat.size(); i++){
        for(auto j = 1; j < mat[0].size(); j++){
            ...
            if(mat[i][j] == 0){
                // get the minimum possible based off max(prev_h, prev_v)+1 and ceiled to parity
                mat[i][j] = max(mat[i-1][j], mat[i][j-1]) + 1;
                if((mat[i][j]&1) ^ par[j%2]) mat[i][j]++;
            } else{
                // if((mat[i][j]&1) ^ par[j%2] || mat[i][j] <= mat[i-1][j] || mat[i][j] <= mat[i][j-1]) {
                if(mat[i][j] <= mat[i-1][j] || mat[i][j] <= mat[i][j-1]) {
                    return 1e18;
                }
            }
...
int main(){
    ...
    if(n == 1 || m == 1){
        ...
        mn = min(min(greedy(mat, 0), greedy(mat, 1)), min(greedy2(mat, 0), greedy2(mat, 1)));
    }
